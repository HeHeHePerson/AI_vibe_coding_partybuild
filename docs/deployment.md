...
系统已内置以下前端安全防护措施（在 `main.js` 中实现）：

1. **禁用右键菜单** - 防止通过右键查看页面源代码
2. **禁用开发者工具快捷键** - 禁用 F12、Ctrl+Shift+I、Ctrl+U 等
3. **禁用页面保存和打印** - 禁用 Ctrl+S、Ctrl+P 等
4. **检测window尺寸变化** - 当开发者工具在侧面打开时，window.innerWidth会变小，系统会检测到异常
5. **检测console输出异常** - 通过重写console.log检测调试行为

**防护原理**：

- 当开发者工具在窗口侧面打开时，`window.outerWidth - window.innerWidth` 会大于160像素
- 系统每500ms检测一次此数值变化
- 如果检测到异常，会记录警告日志

**注意事项**：
- 前端防护只能增加攻击难度，无法完全阻止有经验的黑客
- 攻击者可以通过禁用JavaScript来绕过所有前端防护
...
该脚本会创建所有必要的表结构，包括党建园地相关的表：
- `party_knowledge` (党建知识表)
- `party_activities` (组织活动表)
- `party_activity_participants` (活动参与表)
- `party_fees` (党费缴纳表)
- `party_integral` (党员积分表)
- `party_user_info` (党员信息表)
...
按照前面的部署步骤，部署包含党建园地功能的完整项目代码。

### 13.2 增量部署方案（现有系统升级）

如果现有系统需要升级添加党建园地功能，请按照以下步骤操作：

#### 13.2.1 数据库升级

**重要：请先备份现有数据库，然后执行以下操作**
...
1. **替换或更新项目文件**：
   - `app.py` - 更新路由注册
   - `webapp/routes/` - 添加党建园地相关路由文件
   - `webapp/templates/` - 添加党建园地相关模板
   - `webapp/static/js/main.js` - 更新前端功能

2. **安装依赖**（如果需要）：
   ```bash
   cd /var/www/party-building
   source venv/bin/activate
   pip install flask pymysql bcrypt  # 确保所有依赖已安装
   ```

#### 13.2.3 配置更新

确保配置文件 `config.py` 包含必要的配置项，特别是：
- 数据库连接信息
- 会话配置
- 文件上传配置
...
```bash
# 重启Flask服务
sudo systemctl restart party-building
...
1. **党建知识模块**：
   - 访问 `/party/knowledge` 查看知识列表
   - 点击知识项查看详情
   - 测试学习时间记录和完成功能

2. **组织活动模块**：
   - 访问 `/party/activities` 查看活动列表
   - 测试报名参加活动功能

3. **党费缴纳模块**：
   - 访问 `/party/fees` 查看缴费信息
   - 测试在线缴费功能

4. **党员积分模块**：
   - 访问 `/party/integral` 查看积分概览
   - 查看积分明细和规则
